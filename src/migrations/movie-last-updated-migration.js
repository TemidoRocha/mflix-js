...
;(async () => {
  try {
    const host = process.env.MFLIX_DB_URI // had to replace this by the actual string to work
    const client = await MongoClient.connect(host, {
      useNewUrlParser: true,
    })
    const mflix = client.db(process.env.MFLIX_NS) // // had to replace this by the actual string to work
    ...
    // same as »» const predicate = { lastupdated: { $type: "string" } }
